...
class SqliteStateStorageEngine(SqliteConnection):
    ...
    async def add_new_state_event(self, event: aiomatrix.models.events.RoomStateEvent):
        ...
        params = (
            event.event_id, event.room_id, event.sender, int(event.origin_server_ts.timestamp()), event.type,
            event.json()
        )
        self._make_request(sql, params)

    async def add_new_state_events_batch(self, events: List[aiomatrix.models.events.RoomStateEvent]):
        sql = 'INSERT INTO events (event_id, room_id, sender, ts, event_type, data) VALUES (?, ?, ?, ?, ?, ?)'
        params = [
            (
                event.event_id, event.room_id, event.sender, int(event.origin_server_ts.timestamp()), event.type,
